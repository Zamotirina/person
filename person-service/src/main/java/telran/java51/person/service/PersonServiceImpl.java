...
public Iterable<ChildDto> getChildren() {
	...
	return personRepository.findChildrenBy().map(x->modelMapper.map(x, ChildDto.class)).toList();
}
...
public Iterable<EmployeeDto> getEmployeesBySalaryBetween(int min, int max) {
	...
	return personRepository.findBySalaryBetween(min,max).map(x->modelMapper.map(x, EmployeeDto.class)).toList();
}
